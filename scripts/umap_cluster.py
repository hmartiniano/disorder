...
def make_parser():
    parser = argparse.ArgumentParser(description="Script to cluster protein structures using UMAP and HDBSCAN")
...
def run_experiment(data, universe, n_components, n_neighbors, min_cluster_size):
...
def cluster(data, universe, dataset, n_trials=50):
    print(dataset)
    study_name = dataset.replace(".csv.gz", "") + ".study"
    study = optuna.create_study(direction="minimize", study_name=study_name,
                                storage=f'sqlite:///{dataset}.db', load_if_exists=True)
    ...
    print(df.head())
    df.to_csv(f"{dataset}_clustering.csv", index=False)

    df = df.sort_values(by="value", ascending=False)

    n_components = int(df.iloc[1, :]["params_n_components"])
    n_neighbors = int(df.iloc[0, :]["params_n_neighbors"])
    min_cluster_size = int(df.iloc[0, :]["params_min_cluster_size"])
    d = run_experiment(data, universe, n_components, n_neighbors, min_cluster_size)
    output = pd.DataFrame(d["clusterable_embedding"], index=data.index)
    output["labels"] = d["labels"]
    output.to_csv(f"{dataset}_embeddings.csv")
